...
### Breakable railings and deck grip (`ShipBarrierSystem` + `ShipDeckSurfaceController`)

`ShipHelm.Awake` procedurally builds 28 independently breakable upper-deck railing modules around the authored stepped deck footprint (10 port / 8 starboard / 5 bow / 5 stern), leaving the port ramp opening and starboard stairs clear. Intact modules have a solid full-height blocker; broken modules disable it, hide their horizontal bars, and retain short damaged posts plus a trigger that can still be targeted. Press **E** on any damaged/broken module to repair it; the host validates range and restores the accumulated integrity. Segment IDs, integrity, broken flags, and deck condition ride in every host State packet for self-healing multiplayer sync.

Barrier pressure uses `ShipBalanceController`'s normalized roll/pitch directly (20% lean starts with 20% of weight), reduced by the active deck/cargo grip. Cargo collision/proximity forms a contact graph, so objects pushing through other objects contribute with per-link attenuation even when they do not touch the rail themselves. At `ratedLoad = 50`, a stock rail consumes its full non-regenerating integrity in `ratedHoldSeconds = 300`; lighter loads last proportionally longer, `instantBreakLoad` snaps it immediately, and collision velocity/impulse adds separate impact damage. Only repair resets lost life. Capacity, durability, impact resistance, and repair efficiency expose upgrade multipliers.

`ShipDeckSurfaceController` replaces stock deck friction with authority-only deterministic traction. Dry/Rain/Ice/Sand use progressively different grip coefficients; storms automatically wet a Dry deck, while Ice and Sand are explicit condition hooks. Each `CargoItem` also has category-flavoured grip, impact transfer, and Rigidbody mass derived from gameplay weight, so smooth cargo slides sooner and heavy cargo transmits larger collision impulses.
